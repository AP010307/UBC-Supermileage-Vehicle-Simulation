function [energy1,energy2,energy3,time_req,v_end,energy_end] = fcvLap(max_velocity, initial_velocity, brake_distance, initialOutput, plotFigures)
% fcvLap performs one lap of the FCV simulation
% 
% Input Parameters:
% max_velocity: the maximum velocity for the motor to keep running
% inital velocity: the initial velocity at the beginning of each lap
...
% v_end: the vehicle velocity at the end
...
motorEnergy1_sum = zeros(1,length(x));
...
maxTime = 222.85; % maximum lap time
...
% Watts out interpolation using polynomial fit
...
for i = 2:length(x)
    ...
    % check if vehicle is past the braking distance and has enough energy
    if dist(i)>distToBrake && (kin_energy(i-1)>minEnergy(i-1) && velocity(i-1)>minVel(i-1))
        motorEnergy1_sum(i) = motorEnergy1_sum(i-1);
    ...
    elseif u(i) == 0 % when motor output % is zero
        ...
        % Find torque based on current angular velocity using speed torque slope
        ...
        motorEnergy1_sum(i) = motorEnergy1_sum(i-1) + (motor_output(i)/(drivetrain_eff*motor_eff(i))); % total motor energy input from torque
        ...
        interp_powerIn(2) = u(2)*supplyVoltage*stallCurrent; % Initial power input during start
        motorEnergy2(i) = interp_powerIn(i) * (time(i)-time(i-1)); % motor energy input using interpolated power*time
        motorEnergy2_sum(i) = motorEnergy2_sum(i-1) + motorEnergy2(i);
        
        % Finding power using torque * velocity
        actual_powerOut(i) = u(i)*torque(i) * w(i-1); % calculate power output using torque*velocity
        actual_powerIn(i) = actual_powerOut(i)/(drivetrain_eff*motor_eff(i));
        actual_powerIn(2) = u(2)*supplyVoltage*stallCurrent; % Initial power input during start
        motorEnergy3(i) = actual_powerIn(i) * (time(i)-time(i-1)); % motor energy input using torque*angular velocity
...
for l = 1:length(x)
    pot_energy_sum(l) = pot_energy_sum(l) + minPE;
end
...
energy1 = motorEnergy1_sum(end)/1000; % Motor Input Energy in kJ (from Force*Dist)
energy2 = sum(motorEnergy2)/1000; % Motor Input Energy in kJ (from Power*time)
energy3 = sum(motorEnergy3)/1000;
lapMileage1 = (x(end)/1609) / (motorEnergy1_sum(end)/1000/3600); % Mileage in m/kWh (from Force*Dist)
...
if plotFigures == 1
    ...
    xlim([0 1500]);  xlabel("Distance Travelled [m]"); ylabel("Energy [J]")
    ...
    yyaxis right; 
    plot(dist, motorEnergy1_sum, 'g-', dist, motorEnergy2_sum, 'k-')
    ylabel("Energy [J]");
...
%     hold on;
%     yline(max_velocity);
%     legend('Velocity', 'Error','Integral of Error','Location', 'southoutside');
